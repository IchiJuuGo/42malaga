...
import secrets
# Se utiliza para generar números aleatorios criptográficamente seguros.

from cryptography.fernet import Fernet  
# Genera una clave de cifrado aleatoria que utilizaremos
# para cifrar el archivo que contiene la clave secreta.
...
def generar_clave_secreta():
    # Generamos una clave secreta aleatoria de 64 bytes y devolverla en formato hex.
    clave_bytes = secrets.token_bytes(64)
    clave_hex = clave_bytes.hex()
    return clave_hex

def guardar_clave_en_archivo(clave, nombre_archivo):
    # Guardamos la clave secreta generada, en un archivo cifrado para protegerla.
